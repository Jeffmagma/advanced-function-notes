...
\begin{document}
	...
	The logarithmic function is defined as the inverse of the exponential function. If $f(x)=b^x$, then $f^{-1}(x)=\log_b(x)$, where $b$ is the base of the logarithm. $y=b^2\Leftrightarrow x=\log_bx$. Read: ``log(arithm) (to) base $b$ of $y$''. $y=b^x$ is called the exponential form and $x=\log_by$ is called the logarithmic form.
	\subsubsection{Domain, Range, and Restrictions}
	The domain and range are of the exponential defined by $b^x:$ $(-\infty,+\infty)\to(0,+\infty)$. Since the logarithmic function is the inverse, the domain and range are defined by $\log_bx:$ $(0,+\infty)\to(-\infty,+\infty)$. The base $b$ has the same restrictions as the base of the exponential function: $b>0$, $b\ne1$
	\subsubsection{Basic Formulas}
	\begin{itemize}
		\item $\log_b1=0$
		\item $\log_bb=1$
		\item $\log_b\dfrac{1}{b}=-1$
		\item $\log_{\frac{1}{b}}b=-1$
		\item $\log_bb^n=n$
	\end{itemize}
	\subsubsection{Basic Equations}
	I don't understand what's supposed to go here
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			\addplot+[<->, mark=none, red, samples=128, line width=1pt]{log2(x)};
		\end{axis}
	\end{tikzpicture}
	...
	\end{align*}
	\subsubsection{Earthquakes and Richter Scale}
	Earthquake intensity is measured by the Richter Scale. The formula for the Richter rating of a given quake is given by $\text{R}=\log[I\div I_0]$, where $I_0$ is the intensity of the ``threshold quake'', or movement hat can barely be detected. Other quakes are defined in terms of how many times more intense they are than the threshold quake.
	\subsubsection{Exponential Growth}
	$A=Pe^{kt}$, where $A$ is the current value, $P$ is the initial value, $k$ is the growth factor, and $t$ is the time.
	...
	\end{align*}
	The domain is $x\in\mathbb{R}$ and the range is $f\in\mathbb{R}$.
	\tsubsubsection{Inverse Hyperbolic Cosine Function}
	To get the equation of the inverse of $\cosh x$, you can sub $x$ and $y$ into the definition of $\cosh x$:
	\begin{align*}
		x&=\cosh y\\
		&=\frac{e^y+e^{-y}}{2}\\
		2x&=e^y+e^{-y}\\
	...
	\end{align*}
	The domain is $x\in\mathbb{R}\mid x\ge1$ and the range is $f\in\mathbb{R}\mid y\ge0$.
	\tsubsubsection{Inverse Hyperbolic Tangent Function}
	To get the equation of the inverse of $\tanh x$, you can sub $x$ and $y$ into the definition of $\tanh x$:
	\begin{align*}
		x&=\tanh y\\
		&=\frac{\sinh y}{\cosh y}\\
		&=\frac{e^y-e^{-y}}{e^y+e^{-y}}\\
		&=\frac{e^y-e^{-y}}{e^y+e^{-y}}\times\frac{e^y}{e^y}\\
		&=\frac{e^{2y}-1}{e^{2y}+1}\\
		xe^{2y}+x&=e^{2y}-1\\
		x+1&=e^{2y}(1-x)\\
		e^{2y}&=\frac{1+x}{1-x}\\
	...
	To get the equation of the inverse of $\coth x$, you can sub $x$ and $y$ into the definition of $\coth x$:
	\begin{align*}
		x&=\coth y\\
		&=\frac{\cosh x}{\sinh x}\\
		&=\frac{e^y+e^{-y}}{e^y-e^{-y}}\\
	...
	To find the equation of a tangent line at the point $(a, f(a))$, find the derivative function $f'(x)$, find the slope at $a$ using $m=f'(a)$, then use the slope-point formula to get the equation: $y-f(a)=m(x-a)$
	\subsubsection{Normal Line}
	If $m_T$ is the slope of the tangent line, then the slope of the normal line is given by $m_N=-\dfrac{1}{m_T}$
	\subsubsection{Differentiability for Piecewise Defined Functions}
	For a piecewise defined function: $f(x)=\begin{cases}
		f_1(x)&x<a\\
		c&x=a\\
		f_2(x)&x>a
	\end{cases}$, the function $f$ is differentiable at $x=a$ if the function is continuous at $x=a$, and ${f_1}'(a)={f_2}'(a)$
	\setcounter{subsection}{3}
	\setcounter{subsubsection}{0}
	\tsubsection{AP Component: Product Rule}
	\subsubsection{Product Rule}
	If $f$ and $g$ are differentiable at $x$, then so is $fg$, and $(fg)'=f'g+g'f$
	\subsubsection{Product of Three Functions}
	If $f$, $g$, and $h$ are differentiable at $x$, then so is $fgh$, and $(fgh)'=f'gh+fg'h+fgh'$\\
	Proof: $(fgh)'=(fg)'h+fgh'=(f'g+gf')h+fgh'=f'gh+fg'h+fgh'$
	\subsubsection{Generalized Power Rule}
	If $f$ is differentiable at $x$, then so is $f^n$, and $(f^n)'=nf^{n-1}f'$\\
	Proof for $n=3$: using $(fgh)'=f'gh+fg'h+fgh'$, replace all functions with $f$ to get $f'ff+ff'f+fff'=3f^2f'$
	\setcounter{subsection}{4}
	\setcounter{subsubsection}{0}
	\tsubsection{AP Component: Quotient Rule}
	\subsubsection{Quotient Rule}
	If $f$ and $g$ are differentiable at $x$ and $g(x)\ne0$, then so is $\dfrac{f}{g}$, and $\left(\dfrac{f}{g}\right)'=\dfrac{f'g-g'f}{g^2}$
	\setcounter{subsection}{5}
	\setcounter{subsubsection}{0}
	\tsubsection{AP Component: Chain Rule}
	\subsubsection{Composition of Functions}
	If $u=g(x)$ and $v=f(u)$, then \tikz[baseline]{
		\node (x) at (0,0) {$x$};
		\node (u) at (2,0) {$u$};
		\node (v) at (4,0) {$v$};
		
		\draw[->] (x) -- (u) node[pos=.5, above] {$u=g(x)$};
		\draw[->] (u) -- (v) node[pos=.5, above] {$v=f(u)$};
	} and $v=f(u)=f(g(x))=(f\circ g)(x)$
	\subsubsection{Chain Rule (Leibniz Notation)}
	\tikz[baseline]{
		\node (x) at (0,0) {$\Delta x$};
		\node (u) at (2,0) {$\Delta u$};
		\node (v) at (4,0) {$\Delta v$};
		
		\draw[->] (x) -- (u) node[pos=.5, above] {$u=g(x)$};
		\draw[->] (u) -- (v) node[pos=.5, above] {$v=f(u)$};
	} and $\dfrac{\Delta v}{\Delta x}=\dfrac{\Delta v}{\Delta u}\dfrac{\Delta u}{\Delta x}\ \to\ \dod{v}{x}=\dod{v}{u}\dod{u}{x}$
	...
	\tikz[baseline]{
		...
		\draw[->] (x) -- (u) node[pos=.5, above] {$u=h(x)$};
		\draw[->] (u) -- (v) node[pos=.5, above] {$v=g(u)$};
		\draw[->] (v) -- (w) node[pos=.5, above] {$w=f(v)$};
	} $\dod{w}{x}=\dod{w}{v}\dod{v}{u}\dod{u}{x}$
	\subsubsection{Chain Rule (Lagrange Notation)}
	$v=f(u)=f(g(x))=(f\circ g)(x);\ \dod{v}{x}=(f(g(x)))';\ \dod{v}{u}=f'(u)=f'(g(x));\ \dod{u}{x}=g'(x)$\\
	$\dod{v}{x}=\dod{v}{u}\dod{u}{x}\ \to\ (f(g(x)))'=f'(g(x))g'(x)$\\
	If $g$ is differentiable at $x$ and $f$ is differentiable at $g(x)$, then the composition $(f\circ g)(x)=f(g(X))$ is differentiable at $x$ and $(f\circ g)'(x)=(f(g(x)))'=f'(g(x))g'(x)$. So, the derivative of $f(g(x))$ is the derivative of $f$ evaluated of $g$ multiplied by the derivative of $g$
	\setcounter{section}{5}
	\setcounter{subsection}{1}
	\setcounter{subsubsection}{0}
	...
	The derivative of $e^x$ is $e^x$ itself, proof: $later$ 
	\subsubsection{Derivative of $e^f(x)$}
	Using the chain rule and the fact that ${e^x}'=e^x$, $e^{f(x)\prime}=e^{f(x)}f'(x)$
	\subsubsection{Derivative of $b^x$, $b>0$, $b\ne1$}
	$(b^x)'=b^x\ln b$
	\subsubsection{Derivative of $b^{f(x)}$}
	Using the chain rule, and the fact that $(b^x)'=b^x\ln b$, $(b^{f(x)})'=b^{f(x)}f'(x)\ln b$
	\setcounter{subsection}{2}
	\setcounter{subsubsection}{0}
	\tsubsection{AP Component: Derivatives of Logarithmic Functions}
	\subsubsection{Review of Logarithmic Functions}
	\begin{itemize}
		\item $b^x\Leftrightarrow x=\log_by$
		\item $y=f(x)=\log_bx$, $b>0$, $b\ne1$, $x>0$
		\item $\log_b(xy)=\log_bx+\log_by$
		\item $\log_b\dfrac{x}{y}=\log_bx-\log_by$
		\item $\ln x=\log_ex$
		\item $\log x=\log_{10}x$
		\item $\log_bx^n=n\log_bx$
		\item $\log_bx=\dfrac{\log_ax}{\log_ab}$
		\item $\log_b1=0$
		\item $\log_bb=1$
	\end{itemize}
	\subsubsection{Derivative of $\ln x$}
	$(\ln x)'=\dfrac{1}{x}$, proof
	\subsubsection{Derivative of $\ln f(x)$}
	Using chain rule and $(\ln x)'=\dfrac{1}{x}$, $(\ln f(x))'=\dfrac{f'(x)}{f(x)}$
	\subsubsection{Derivative of $\log_bx$}
	$(\log_bx)'=\dfrac{1}{x\ln b}$, proof
	\subsubsection{Derivative of $\log_bf(x)$}
	Using chain rule and $(\log_bx)'=\dfrac{1}{x\ln b}$, $(\log_bf(x))'=\dfrac{f'(x)}{f(x)\ln b}$
	\setcounter{subsection}{4}
	\setcounter{subsubsection}{0}
	\tsubsection{AP Component: Derivatives of Trigonometric Functions}
	\subsubsection{Review of Trigonometric Functions}
	\begin{itemize}
		\item $\sin x:\mathbb{R}\to[-1,1]$
		\item $\cos x:\mathbb{R}\to[-1,1]$
		\item $\sin(x+\dfrac{\pi}{2})=\cos x$
		...
		\item $\tan x:\set{x\in\mathbb{R}\mid x\ne\dfrac{\pi}{2}+n\pi,n\in\mathbb{Z}}\to\mathbb{R}$
	\end{itemize}
	\subsubsection{Derivate of $\sin x$}
	$(\sin x)'=\cos x$
	\subsubsection{Derivative of $\sin f(x)$}
	$(\sin f(x))'=(\cos f(x))f'(x)$
	\subsubsection{Derivative of $\cos x$}
	$(\cos x)'=-\sin x$
	\subsubsection{Derivative of $\cos f(x)$}
	$(\cos f(x))'=-(\sin f(x))f'(x)$
	\subsubsection{Derivative of $\tan x$}
	$(\tan x)'=\sec^2(x)$
	\setcounter{section}{1}
	\setcounter{subsection}{5}
	\setcounter{subsubsection}{0}
	\tsubsection{AP Component: Tangent and Normal Lines}
	\subsubsection{Lines}
	The equation of a non vertical line in slope y-intercept form is $y=mx+b$, where $m$ is the slope and $b$ is the y-intercept. If two points $P_1(x_1,y_1)$ and $P_2(x_2,y_2)$ are on a line, the slope of the line is given by $m=\dfrac{y_2-y_1}{x_2-x_1}$. If given the slope of the line and a point $P(x_1,y_1)$ on the line, then the equation of the line is $y-y_1=m(x-x_1)$
	\subsubsection{Parallel and Perpendicular Lines}
	The slopes of two parallel lines are equal ($m_1=m_2$). The slopes of two perpendicular lines are negative reciprocal $\left(m_1=-\dfrac{1}{m_2}\text{ or }m_1m_2=-1\right)$
	\subsubsection{Tangent Line}
	Consider a function $y=f(x)$ and a point $P(a, f(a))$ on its graph. The slope of the tangent line passing through $P$ is given by $m=\lim\lim\limits_{x\to a}\dfrac{f(x)-f(a)}{x-a}$ or $m=\lim\lim\limits_{x\to a}\dfrac{f(x)-f(a)}{x-a}$. The equation of a tangent line passing through $P$ and having slope $m$ is $y-f(a)=m(x-a)$
	\subsubsection{Normal Line}
	Consider a function $y=f(x)$ and a point $P(a, f(a))$ on its graph. The normal line passing through the point $P$ is perpendicular to the tangent line passing through the same point. The equation of the normal line passing through the point $P(a, f(a))$ where the slope of the tangent line is $m$ is given by $y-f(a)=-\dfrac{1}{m}(x-a)$
	...
	Let us consider a 3D coordinate system and a 3D vector $\vv{v}$ with a tail at the origin $O$. Parallel planes through its tip to the coordinate planes intersect the x-axis at $v_x$, the y-axis at $v_y$, and the z-axis at $v_z$. The triple $(v_x,\,v_y,\,v_z)$ determines the scalar components of the 3D vector and $\vv{v}=(v_x,\,v_y,\,v_z)$. %graph
	...
	The unit vectors $\vv{i}=(1,\,0,\,0)$, $\vv{j}=(0,\,1,\,0)$, and $\vv{k}=(0,\,0,\,1)$ are called the standard unit vectors in 3D space. %graph
	...
	The midpoint of the line segment $AB$ is the point $M$ such that $\vv{MA}+\vv{MB}=0$
	\subsubsection{Centroid}
	The centroid of a system of point $P_1,P_2,\dots,P_n$ is a point $C$ defined by $\vv{OC}=\dfrac{\vv{OP_1}+\vv{OP_2}+\dots+\vv{OP_n}}{n}$
	\subsubsection{Parallelism}
	Two vectors $\vv{a}$ and $\vv{b}$ are parallel ($\vv{a}\|\vv{b}$) if there exists $\lambda$ such that $\vv{a}=\lambda\vv{b}$
	\subsubsection{Co-Linearity}
	Three points $A$, $B$, and $C$ are collinear if $\vv{AB}\|\vv{BC}$
	\subsubsection{Linear Dependency}
	Three vectors $\vv{a}$, $\vv{b}$, and $\vv{c}$ are linear dependent if there exists $\lambda$ and $\mu$ such that $\vv{c}=\lambda\vv{a}+\mu\vv{b}$
